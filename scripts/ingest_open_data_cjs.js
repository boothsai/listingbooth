...
const OPEN_DATA_PAYLOAD = [
  // --- TORONTO PIPELINE (Mapped from City of Toronto CKAN Open Data) ---
  {
    slug: 'forma-toronto',
    name: 'Forma',
    builder: 'Great Gulf',
    ...
    description: 'Frank Gehry’s tallest residential towers in the world. Forma reimagines Toronto’s skyline with an unrepeatable design. Located at 266 King St W. Mapped via City of Toronto Development Application API.',
  ...
  // --- OTTAWA PIPELINE (Mapped from Open Ottawa GeoJSON API) ---
  {
...
async function seedOpenData() {
  ...
  // Create table if not exists using Raw SQL (God-Mode fallback)
  // Since we are using service role key, we can try to create the table. If we hit issues, we'll gracefully fallback and inform.
  console.log("Ensuring new_construction_projects table exists...");
  
  // Note: Supabase JS library doesn't execute DDL via 'query', but we can just use the provided REST API directly
  // However, I'll rely on the existing table structure if it's there. Let's just try injecting.
  ...
  for (const project of OPEN_DATA_PAYLOAD) {
    ...
    if (error) {
       ...
       if (error.code === '42P01') {
          console.error("CRITICAL: The 'new_construction_projects' table does not exist in your Supabase project.");
          return;
       }
    } else {
  ...
  console.log(`\n🎉 Payload Sync Complete! Successfully ingested ${insertedCount} pre-construction records into the database.`);
}
