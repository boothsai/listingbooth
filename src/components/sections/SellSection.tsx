    ...
    <section id="sell" style={{ backgroundColor: '#fafafa', padding: '80px 0', borderTop: '1px solid #f0f0f0' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 40px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '80px', alignItems: 'center' }}>
          ...
          {/* Right: Why sell stats */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            {[
              { value: '14 days', label: 'Average time to sell', sub: 'vs. 28-day industry average', icon: '⚡' },
              { value: '102%', label: 'Sale-to-list price ratio', sub: 'Our listings close above asking', icon: '📈' },
              { value: '$0', label: 'Upfront listing cost', sub: 'Pay only when your home sells', icon: '💰' },
              { value: '3,200+', label: 'Successful closings', sub: 'Across Canada & the US', icon: '🏠' },
            ].map(s => (
              <div key={s.label} style={{
                background: 'white', borderRadius: '16px', padding: '24px 28px',
                border: '1.5px solid #eee', display: 'flex', alignItems: 'center', gap: '20px',
              }}>
                <div style={{ fontSize: '32px', flexShrink: 0 }}>{s.icon}</div>
                <div>
                  <p style={{ margin: '0 0 2px', fontSize: '30px', fontWeight: 900, letterSpacing: '-1px', color: '#111', lineHeight: 1 }}>{s.value}</p>
                  <p style={{ margin: '0 0 4px', fontSize: '14px', fontWeight: 700, color: '#333' }}>{s.label}</p>
