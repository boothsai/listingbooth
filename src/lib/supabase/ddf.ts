...
function getBase() {
  if (!_base) {
    // Lazily read env vars at runtime, not at module import time (prevents build crash)
...
export interface DdfListing {
  ...
  features: string[] | null; // Gemini Vision extracted JSON tags
}
...
export interface ListingSearchParams {
  ...
  features?: string[]; // Array of Gemini Vision semantic tags
  page?: number;
  limit?: number;
}
...
export async function searchListings(params: ListingSearchParams): Promise<ListingSearchResult> {
  ...
  if (params.features && params.features.length > 0) {
    // PostgREST array containment filter: returns rows where 'features' column contains all specified tags
